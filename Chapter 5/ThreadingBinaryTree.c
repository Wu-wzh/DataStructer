...
ThreadNode *InPreNode(ThreadNode *p){
    ...
    //若左节点不是后继节点，则寻找左子树中第一个访问的元素，即为此节点的前驱节点
    //而寻找左子树第一个访问的元素便可调用上面的lastNode函数
...
/*★★★★总结:
线索中序遍历时，某个节点是在左节点和右节点之间访问的，这种特性导致了找这个中序线索树某节点的前驱和后继节点不费事，
当tag不为1时，其前驱和后继都在以此节点为根的子树中，不会位于此节点上方

线索前序遍历时，情况有所不同，tag不为1时，前序线索树的后继节点好找，肯定在此节点为根的子树里，既在此节点下方
但是寻找前驱节点之后，由于前序遍历的特性，当tag不为1时，其前驱节点一定在这个节点的上方，但是二叉链表是难以查询其父节点的，因此需要遍历一颗个树来查找他的父节点
此时我们说实现不了前驱节点的寻找

线索后序遍历和前序遍历类似，互相颠倒，寻找前驱节点较为简单，肯定在节点上方，但是后继节点的寻找，在节点的上方，所以需要访问节点的父节点，也需要遍历一整棵树才能找到
我们说实现不了后继节点的寻找

解决前序前驱节点和后续后继节点的寻找，可以使用三叉链表的存储结构，记录节点的父节点，这样就不用遍历整颗树来寻找父节点了，便可实现三种线索树的遍历和逆序遍历
*/
int main(){
    ThreadTree T;
    CreateTree(&T);
